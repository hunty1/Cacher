...
def cacher(lines, targetDate, friendlyNames, site_name, measurement):
    ...
    finalOutput.append('  %s Unique LAN IP Addresses.'
                       % len(set(lan_ip_addresses)))
    if len(doe_proxy_requests) > 0:
        ...
        finalOutput.append(
            ...
            '  This will adversely affect your bandwidth usage.'
            % len(doe_proxy_requests))
    finalOutput.append('')
...
def send_serveralert(targetDate, cacherdata, site_name):
    try:
        ...
        cmd = ['/Applications/Server.app/Contents/ServerRoot/usr/sbin/server '
               'postAlert CustomAlert Common subject ' + '"' + site_name +
...
def main():
    ...
    site_name_cmd = "defaults read /Library/Preferences/com.apple.RemoteDesktop Text1 | awk -F \": \" \'{print $3}\' | awk -F \",\" \'{print $1}\'"  # nopep8
    ...
    cacherdata = cacher(
        rawLog.readlines(), targetDate, friendlyNames, site_name, measurement)
    # Output conditionals
    if stdOut:
        print("\n".join(cacherdata))
    if slackAlert:
